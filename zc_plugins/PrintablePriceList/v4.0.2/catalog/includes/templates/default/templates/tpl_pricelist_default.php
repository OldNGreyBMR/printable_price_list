 ...
 * @copyright parts Copyright 2003-2026 Zen Cart Development Team
 * @license http://www.zen-cart.com/license/2_0.txt GNU Public License V2.0
 * @version 4.0.2
 * @author Updated by OldNGrey
 * @zencart v1.5.8a to v2.3.0
 *
 * Last updated: v4.0.2
 *  2026-09-06 print opens in new page
 */
global $price_list, $template, $current_page_base, $zcDate, $db;

$home_link = zen_href_link(FILENAME_DEFAULT);
$store_title_link = '<a href="' . $home_link . '">' . TITLE . '</a>';
$header_logo = '<a href="' . $home_link . '">' . zen_image($template->get_template_dir(HEADER_LOGO_IMAGE, DIR_WS_TEMPLATE, $current_page_base, 'images') . '/' . HEADER_LOGO_IMAGE, HEADER_ALT_TEXT) . '</a>';
...
<div class="noPrintPL">
    <div id="screenIntroPL">
        <?php if ($price_list->config['show_logo']) { ?>
        <?= $header_logo ?>
        <?php } ?>
        <h3><?= sprintf(TEXT_PL_HEADER_TITLE, $store_title_link) ?></h3>
...
?>
            <td>
                ...
                <div id="categoriesPLContent" class="sideBoxContent centeredContent">
                     <?= zen_draw_label(PLEASE_SELECT, 'select-category', 'class="sr-only"'); //
                   ?>
                    <?=
...
?>
    <?= PL_TEXT_GROUP_NOT_ALLOWED ?>
<?php
    if (zen_is_logged_in() && !zen_in_guest_checkout()) {
?>
    <a href="<?= zen_href_link(FILENAME_LOGOFF, '', 'SSL') ?>"><?= HEADER_TITLE_LOGOFF ?></a>
<?php
    } elseif (STORE_STATUS === '0') {
?>
    &nbsp;(<a href="<?= zen_href_link(FILENAME_LOGIN, '', 'SSL') ?>"><?= HEADER_TITLE_LOGIN ?></a>)
<?php
    }
} else {
    if (count($price_list->rows) === 0) {
?>
    <h3 id="noMatchPL"><?= TEXT_PL_NOTHING_FOUND ?></h3>
<?php
    } else {
?>
    <table class="tablePL">
...
?>
                    <div class="headPL">
                        <?= $header_logo ?>
                        <h4 class="headerTitlePrintPL"><?= sprintf(TEXT_PL_HEADER_TITLE_PRINT, $store_title_link) ?></h4>
                    </div>
<?php
                    }
?>
                    <div class="datePL"><?= $zcDate->output(DATE_FORMAT_LONG) ?></div>
                    <div id="print-me"><a href="<?= zen_href_link(FILENAME_PRICELIST, zen_get_all_get_params()) ?>" target="_blank" onclick="var pplWin=window.open(this.href,'pplPrintWindow');if(pplWin){pplWin.onload=function(){pplWin.focus();pplWin.print();}}return false;"><?= PL_PRINT_ME ?></a></div>
...
?>
                    <?= zen_image(DIR_WS_IMAGES . $current_row['products_image'], $products_name, $price_list->config['image_width'], $price_list->config['image_height'], 'class="imgPL"') ?>
<?php
                    }
?>
                    <?= $products_name ?>
...
?>
                                    <?= $separator . $option_value_name . $option_value_price . $price_suffix ?>
<?php
                                $separator = ', ';
                            }
?>
...
<?php
                if ($price_list->config['show_price']) {
?>
                <td class="splPL"><?= $special_price_inc ?></td>
<?php
                }
                if ($price_list->config['show_taxfree']) {
?>
                <td class="splPL"><?= $special_price_ex ?></td>
<?php
                }
?>
            </tr>
...
?>
                    <?= zen_clean_html($current_row['products_description']) ?><a href="<?= zen_href_link($products_info_page, 'products_id=' . $products_id) ?>"><?= MORE_INFO_TEXT ?></a>
<?php
                } else {
?>
                    <?= $current_row['products_description'] ?>
<?php
                }
?>
                </td>
